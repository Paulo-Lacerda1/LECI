...
Graph* GraphCreateTranspose(const Graph* g) {
    assert(g != NULL);
    assert(g->isDigraph);       // A transposição só é válida para grafos orientados
    assert(g->isComplete == 0); // Não suportado para grafos completos

    // Criar o grafo transposto
    Graph* gTranspose = GraphCreate(g->numVertices, g->isDigraph, g->isWeighted);
    if (gTranspose == NULL) {
        return NULL;
    }

    // Criar arrays temporários para armazenar os novos graus
    unsigned int* newInDegrees = calloc(g->numVertices, sizeof(unsigned int));
    unsigned int* newOutDegrees = calloc(g->numVertices, sizeof(unsigned int));
    if (newInDegrees == NULL || newOutDegrees == NULL) {
        free(newInDegrees);
        free(newOutDegrees);
        GraphDestroy(&gTranspose);
        return NULL;
    }
    ...
    while (ListCurrentIsInside(g->verticesList)) {
        struct _Vertex* srcVertex = (struct _Vertex*)ListGetCurrentItem(g->verticesList);
        ListMoveToHead(srcVertex->edgesList);

        // Para cada aresta, criar uma inversa no transposto
        while (ListCurrentIsInside(srcVertex->edgesList)) {
            struct _Edge* edge = (struct _Edge*)ListGetCurrentItem(srcVertex->edgesList);

            // Procurar o vértice de destino no grafo transposto
            ListSearch(gTranspose->verticesList, &(edge->adjVertex));
            struct _Vertex* destVertex = (struct _Vertex*)ListGetCurrentItem(gTranspose->verticesList);

            // Criar uma nova aresta no grafo transposto
            ...
            newEdge->adjVertex = srcVertex->id; // Inverter a direção
            newEdge->weight = edge->weight;

            if (ListInsert(destVertex->edgesList, newEdge) != 0) {
                free(newEdge);
                free(newInDegrees);
                free(newOutDegrees);
                GraphDestroy(&gTranspose);
            ...
            // Atualizar os graus temporariamente
            newOutDegrees[edge->adjVertex]++;
            newInDegrees[srcVertex->id]++;

            // Incrementar o número de arestas no grafo transposto
            gTranspose->numEdges++;

            ListMoveToNext(srcVertex->edgesList);
        }
        ListMoveToNext(g->verticesList);
    }

    // Aplicar os novos graus acumulados
    ListMoveToHead(gTranspose->verticesList);
    unsigned int i = 0;
    while (ListCurrentIsInside(gTranspose->verticesList)) {
        struct _Vertex* vertex = (struct _Vertex*)ListGetCurrentItem(gTranspose->verticesList);

        vertex->inDegree = newInDegrees[i];
        vertex->outDegree = newOutDegrees[i];

        i++;
        ListMoveToNext(gTranspose->verticesList);
    }
