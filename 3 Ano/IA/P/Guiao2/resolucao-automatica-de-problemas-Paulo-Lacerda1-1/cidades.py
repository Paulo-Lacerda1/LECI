...
class Cidades(SearchDomain):
    ...
    def cost(self, city, action):
        (C1, C2) = action
        # distância entre C1 e C2
        for (A, B, D) in self.connections:
            if (A == C1 and B == C2) or (A == C2 and B == C1):
                return D
        return None  # Se não encontrar
    
    def heuristic(self, city, goal_city):
        # Heurística: distância Euclidiana entre as coordenadas das cidades
        if city not in self.coordinates or goal_city not in self.coordinates:
            return 0
