...
void insert(SllNode*& first, const Student& newStudent) {
    SllNode* newNode = new SllNode{newStudent, nullptr};
    if (!first) {
        first = newNode; // Se a lista estiver vazia, o novo nó será o primeiro
    } else {
        SllNode* temp = first;
        while (temp->next) {
            temp = temp->next; // Vai até o último nó
        }
        temp->next = newNode; // Insere no final
    }
}
