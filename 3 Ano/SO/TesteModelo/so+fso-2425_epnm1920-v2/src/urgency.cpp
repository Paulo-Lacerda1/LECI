...
void init_simulation(int np)
{
   ...
   hd->num_patients = np;
   hd->num_doctors  = ndoctors;     //guarda o número de médicos
...
int nurse_iteration(int id) // return value can be used to request termination
{
   ...
   if (patient == 0 || patient>100) {
      printf("\e[34;01mNurse %d: no more patients, finishing shift\e[0m\n", id);
      return 1;
   }
...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   if(patient < 0 ){
        printf("\e[32;01mDoctor %d: no more patients, finishing shift\e[0m\n", id);
      return 1;
   }
   ...
   cond_broadcast(&hd->all_patients[patient].finish);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   while (hd->all_patients[id].done == 0)
    cond_wait(&hd->all_patients[id].finish, &hd->all_patients[id].mutex);
...
{
    // converte o argumento genérico para inteiro
    int id = *(int*)arg;

    patient_goto_urgency(id);

    mutex_lock(&hd->all_patients[id].mutex);    //Aceder ao mutex do paciente[i]

    while (hd->all_patients[id].done == 0){      //Enquanto o médico ainda não sinalizou o fim da consulta (done == 0)
        cond_wait(&hd->all_patients[id].finish, &hd->all_patients[id].mutex);
    }
...
void* nurse_life(void* arg)
{
   ...
   while (1) {
      int end = nurse_iteration(id);
      if (end != 0)   // se a nurse_iteration devolveu 1
            break;
   }
...
void* doctor_life(void* arg)
{
    ...
    while (1) {
        int end = doctor_iteration(id);
        if (end != 0) // se devolveu 1 → terminar
            break;
    }
...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0;i < nnurses; i++){           //threads para cada enfermeira
      nurse_ids[i] = i;
      thread_create(&nurses[i],NULL, nurse_life, &nurse_ids[i]);
   }
   ...
   for (int i = 0; i < npatients; i++) {
      patient_ids[i] = i;
      thread_create(&patients[i], NULL, patient_life, &patient_ids[i]);
      random_wait(); // pequeno atraso na chegada para nao dar barraco
   }
   ...
   // Criar threads de médicos
   for (int i = 0; i < ndoctors; i++) {
       doctor_ids[i] = i;
       thread_create(&doctors[i], NULL, doctor_life, &doctor_ids[i]);
   }

   for(int i=0; i < npatients; i++){      //espera todos os pacientes acabarem 
      thread_join(patients[i], NULL);
   }
   
   close_pfifo(&hd->triage_queue);

   for (int i = 0; i < nnurses; i++)      //espera todos os enfermeiros acabarem 
...
void new_patient(Patient* patient)
{
   ...
   patient->done = 0;
   mutex_init(&patient->mutex, NULL);     //INICIAR O MUTEX
   cond_init(&patient->finish, NULL);     //INICIAR A CONDICAO      
}
